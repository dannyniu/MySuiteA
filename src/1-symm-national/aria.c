...
#define CALC(u,v,i)                                                     \
    ((uint32_t)u << 24 | (uint32_t)v << 16 | (uint32_t)i * 0x0101),     \
        ((uint32_t)i * 0x01010000 | (uint32_t)u << 8 | (uint32_t)v)

static const alignas(1024) uint32_t table_SB[512] = SBOX_WIDENER2(
    CALC,
...
static void SL1(uint8_t x[16])
{
    ...
    for(i=0; i<4; i++)
    {
        y = be32toh(((uint32_t *)x)[i]);
        y = wsbox(y, table_SB);
        ((uint32_t *)x)[i] = htobe32(y);
    }
}
