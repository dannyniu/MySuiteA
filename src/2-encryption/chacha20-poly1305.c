...
void ChaCha_AEAD_Encrypt(chacha_aead_t *restrict x,
                         ...
                         size_t tlen, void *T)
{
    alignas(uint32_t) uint8_t words[32];
    ...
    for(i=0; i<alen; i+=16) {
        for(j=0; j<16; j++) words[j] = i+j<alen ? ((uint8_t *)aad)[i+j] : 0;
        poly1305_1block(&x->poly1305, words);
    }

    for(i=0; i<len; i+=16) {
        for(j=0; j<16; j++) words[j] = i+j<len ? ((uint8_t *)out)[i+j] : 0;
        poly1305_1block(&x->poly1305, words);
    }
                          ...
                          size_t tlen, const void *T)
{
    ...
    for(i=0; i<len; i+=16) {
        for(j=0; j<16; j++) words[j] = i+j<len ? ((uint8_t *)in)[i+j] : 0;
        poly1305_1block(&x->poly1305, words);
    }
