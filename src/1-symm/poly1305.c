...
void poly1305_init(poly1305_t *restrict poly1305, uint8_t key[32])
{
    ...
    for(i=0; i<4; i++) {
        poly1305->r[i] = le32toh(((uint32_t *)key)[i]);
        poly1305->s[i] = le32toh(((uint32_t *)key)[i+4]);
    }
...
void poly1305_1block(poly1305_t *restrict poly1305, const void *restrict data)
{
    if( data )
    {
        ...
        p1305bn_addto(poly1305->a, le32toh(((uint32_t *)data)[0]), 0);
        p1305bn_addto(poly1305->a, le32toh(((uint32_t *)data)[1]), 1);
        p1305bn_addto(poly1305->a, le32toh(((uint32_t *)data)[2]), 2);
        p1305bn_addto(poly1305->a, le32toh(((uint32_t *)data)[3]), 3);
